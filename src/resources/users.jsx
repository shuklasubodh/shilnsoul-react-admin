...
const UserList = () => <List filters={[<SearchInput key="search" source="q" alwaysOn />]}><Datagrid rowClick="show">
  ...
  <TextField source="country_code" label="Country code" /><TextField source="phone" /><TextField source="role" /><BooleanField source="is_active" /><DateField source="created_at" showTime /><EditButton />
</Datagrid></List>
const UserForm = ({ creating = false }) => <SimpleForm>
  ...
  <TextInput source="email" type="email" validate={required()} /><CountryCodeInput />
  <TextInput source="phone" label="Mobile number (local or E.164)" helperText="Example: 90032305; the selected calling code is added automatically." validate={required()} />
...
const UserShow = () => <Show><SimpleShowLayout>
  <TextField source="id" /><TextField source="first_name" /><TextField source="last_name" /><EmailField source="email" />
  <TextField source="country_code" label="Country code" /><TextField source="phone" /><TextField source="role" /><BooleanField source="is_active" />
